...
def login2(request):
    ...
    if limit=="用户":
        ...
        if mess[1] == 0:
            username = Control.cons.get(conid=id).conname
            num = len(Usercom.usercoms.referone(id))
    ...
    if limit=="用户":
        ...
        res.set_cookie("conid", id, max_age=600000)
        ses = request.session["conid"] = id
...
def fish(request):
    ...
    if var == 0:
        ...
        try:
            ...
            num = 0
            loginmess = "切换用户"
            username = Control.cons.get(conid=conid).conname
...
def dell(request,comid):
    cookie = request.COOKIES
    conid = cookie.get("conid")
    Commodity.coms.deleteone(comid)
    username = Control.cons.get(conid=conid).conname
    num=0
    return render(request, "index.html", {"num": num, "loginmess": "切换用户", "username": username,
                                          "titlelist": titlelist,
...
def buycom(request):
    ...
    return render(request,"buycom.html",
                  {"num":num,"comimg":comimg,"comid":comid,"comname":comname,"comdescribe":comdescribe,
                   "commoney":commoney,"complace":complace,"usertel":usertel,"userqq":userqq,"var":var})
