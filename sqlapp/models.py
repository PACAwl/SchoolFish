...
class UserManager(models.Manager):
    ...
    def register(self, id, password, password2, name):
        idlist=self.ids()
        if len(str(id))<8 or len(str(id))>16:
            return ["register.html",5]
        elif len(password)<8 or len(password)>16:
            return ["register.html",6]
        elif re.match(r'^[0-9a-zA-Z]+$', password) == None:
            return ["register.html",7]
        elif id in idlist:
            return ["register.html",1]
        elif password!=password2:
            return ["register.html",2]
